...
const sqlite_migrator = @import("sqlite_migrator.zig");

test "sqlite_migrator: migrate existing SQLite database to pure-Zig SratimDB" {
    const testing = std.testing;
    const allocator = testing.allocator;

    const snap_path = "tmp/migrated_sratim.json";
    const wal_path = "tmp/migrated_sratim.wal";
    const logs_snap_path = "tmp/migrated_logs.json";
    const logs_wal_path = "tmp/migrated_logs.wal";

    defer std.Io.Dir.cwd().deleteFile(testing.io, snap_path) catch {};
    defer std.Io.Dir.cwd().deleteFile(testing.io, wal_path) catch {};
    defer std.Io.Dir.cwd().deleteFile(testing.io, logs_snap_path) catch {};
    defer std.Io.Dir.cwd().deleteFile(testing.io, logs_wal_path) catch {};

    var storage = engine.SratimStorage.init(allocator, testing.io, snap_path, wal_path);
    defer storage.deinit();

    var logs_storage = logs_engine.LogsStorage.init(allocator, testing.io, logs_snap_path, logs_wal_path);
    defer logs_storage.deinit();

    // Copy real sratim.db to tmp/ for safe isolated testing
    const db_data = std.Io.Dir.cwd().readFileAlloc(testing.io, "sratim.db", allocator, std.Io.Limit.limited(20 * 1024 * 1024)) catch return;
    defer allocator.free(db_data);
    {
        const out_f = std.Io.Dir.cwd().createFile(testing.io, "tmp/source_sratim.db", .{}) catch return;
        defer out_f.close(testing.io);
        var buf: [65536]u8 = undefined;
        var f_w = out_f.writer(testing.io, &buf);
        try f_w.interface.writeAll(db_data);
        try f_w.interface.flush();
    }
    defer std.Io.Dir.cwd().deleteFile(testing.io, "tmp/source_sratim.db") catch {};
    defer std.Io.Dir.cwd().deleteFile(testing.io, "tmp/source_sratim.db.migrated") catch {};
    defer std.Io.Dir.cwd().deleteFile(testing.io, "tmp/source_sratim.db-shm") catch {};
    defer std.Io.Dir.cwd().deleteFile(testing.io, "tmp/source_sratim.db-wal") catch {};

    const logs_data = std.Io.Dir.cwd().readFileAlloc(testing.io, "logs.db", allocator, std.Io.Limit.limited(20 * 1024 * 1024)) catch return;
    defer allocator.free(logs_data);
    {
        const out_f = std.Io.Dir.cwd().createFile(testing.io, "tmp/source_logs.db", .{}) catch return;
        defer out_f.close(testing.io);
        var buf: [65536]u8 = undefined;
        var f_w = out_f.writer(testing.io, &buf);
        try f_w.interface.writeAll(logs_data);
        try f_w.interface.flush();
    }
    defer std.Io.Dir.cwd().deleteFile(testing.io, "tmp/source_logs.db") catch {};
    defer std.Io.Dir.cwd().deleteFile(testing.io, "tmp/source_logs.db.migrated") catch {};
    defer std.Io.Dir.cwd().deleteFile(testing.io, "tmp/source_logs.db-shm") catch {};
    defer std.Io.Dir.cwd().deleteFile(testing.io, "tmp/source_logs.db-wal") catch {};

    // Migrate from the tmp copies
    try sqlite_migrator.migrateIfNeeded(allocator, testing.io, &storage, &logs_storage, "tmp/source_sratim.db", "tmp/source_logs.db");

    if (storage.countMovies() > 0) {
        try testing.expect(storage.countLibraries() > 0);
        std.debug.print("\n[Migrator Test] Successfully imported: {d} movies, {d} shows, {d} episodes, {d} users\n", .{
            storage.countMovies(),
            storage.countShows(),
            storage.countEpisodes(),
            storage.countUsers(),
        });
    }
}
