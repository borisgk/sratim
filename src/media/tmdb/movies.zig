...
pub fn fetchMovieCredits(
    ...
    const fetch_url = try std.fmt.allocPrint(allocator, "https://api.themoviedb.org/3/movie/{d}/credits", .{tmdb_id});
    defer allocator.free(fetch_url);

    std.debug.print("TMDB Movie Credits Request URL: {s}\n", .{fetch_url});
    ...
    std.debug.print("TMDB Movie Credits Response Status: {d}\n", .{response.status.code});
