...
fn calculateStats(native: []const f32, ffmpeg: []const f32, sample_rate: u32, num_segments: usize, allocator: std.mem.Allocator) ![]SegmentStats {
    ...
    for (0..num_segments) |seg_idx| {
        ...
        for (nat_slice, ff_slice) |raw_n, f| {
            const n = raw_n * 2.0; // Scale to match FFmpeg full scale output
        ...
        const corr = if (denom > 1e-12) dot / denom else 0.0;
...
fn printTerminalReport(label: []const u8, file_path: []const u8, overall: SegmentStats, segments: []const SegmentStats, success_frames: usize, failed_frames: usize) void {
    std.debug.print("\n", .{});
    std.debug.print("====================================================================================================\n", .{});
    std.debug.print("   EAC-3 DECODING COMPARISON REPORT ({s}): Pure Zig Eac3Decoder vs FFmpeg Reference\n", .{label});
...
fn generateHtmlReport(
    ...
    try w.writeAll(
        ...
        \\<head>
        \\<meta charset="UTF-8">
        \\<title>EAC-3 Decoding Oscilloscope Comparison</title>
        \\<style>
    ...
    try w.writeAll("  <h1>EAC-3 Waveform Comparison: Pure Zig Eac3Decoder vs FFmpeg Reference (");
    try w.writeAll(label);
    try w.writeAll(")</h1>\n");
    try w.writeAll("  <div class=\"subtitle\">Input File: <code>");
    try w.writeAll(file_path);
    try w.writeAll("</code> (48,000 Hz, EAC-3 ");
    try w.writeAll(label);
    try w.writeAll(")</div>\n");
    ...
    if (overall.correlation >= 0.90) {
        ...
        try w.writeAll(corr_str);
        try w.writeAll(" <span class=\"badge amber\">IN PROGRESS</span></div></div>");
    }
    ...
    const snr_str = std.fmt.bufPrint(&snr_buf, "{d:.2} dB", .{overall.snr_db}) catch "0 dB";
    ...
    const frame_str = std.fmt.bufPrint(&frame_buf, "{d} ok / {d} err", .{ success_frames, failed_frames }) catch "N/A";
    try w.writeAll("<div class=\"card-val\">");
    try w.writeAll(frame_str);
    try w.writeAll("</div></div>");
    ...
    const dur_str = std.fmt.bufPrint(&dur_buf, "{d:.2}s ({d} samples)", .{ overall.end_time, overall.sample_count }) catch "10s";
    try w.writeAll("<div class=\"card-val\">");
    ...
    try w.writeAll(
        ...
        \\      </div>
        \\      <div class="legend">
        \\        <div class="legend-item"><div class="legend-line" style="background:#38bdf8;"></div> <strong>Pure Zig Eac3Decoder</strong></div>
    ...
    try w.writeAll(
        ...
        \\      ctx.fillText('Pure Zig Eac3Decoder (' + currentChannel.toUpperCase() + ')', 20, 22);
        ...
        \\      ctx.fillText('Overlaid Comparison: ' + count + ' Samples (' + (count/48).toFixed(2) + ' ms)', 20, 22);
        \\
        \\      ctx.strokeStyle = '#38bdf8';
        \\      ctx.lineWidth = 2.2;
        \\      ctx.beginPath();
        ...
        \\      const midY = H * 0.5;
        \\      const scaleY = (H * 0.40) / (maxAmp * 0.5);
        \\      ctx.strokeStyle = '#141e33';
        \\      ctx.lineWidth = 1;
...
pub fn runEac3Test(
    ...
    var decoder = eac3_dec.Eac3Decoder.init();
    ...
    if (first_err) |err| {
        std.debug.print("[EAC-3 Decoder Notice] Frame decode encountered error: {} (Failed frames: {d}/{d})\n", .{ err, failed_frames, success_frames + failed_frames });
    }
    ...
    const corr_l = if (denom_l > 1e-12) dot_l / denom_l else 0.0;
    const corr_r = if (denom_r > 1e-12) dot_r / denom_r else 0.0;
