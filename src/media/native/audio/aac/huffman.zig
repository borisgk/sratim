...
pub fn buildTrie(comptime codes: []const u32, comptime bits: []const u5) [1024]TrieNode {
    ...
    for (codes, bits, 0..) |code, len, sym| {
        ...
        while (bit_idx > 0) {
            bit_idx -= 1;
            const b: u1 = @intCast((code >> @intCast(bit_idx)) & 1);
            if (bit_idx == 0) {
                trie[curr].child[b] = 0x8000 | @as(u16, @intCast(sym));
            } else {
                var next = trie[curr].child[b];
                if (next == 0) {
                    next = next_node;
                    next_node += 1;
                    trie[curr].child[b] = next;
                }
                curr = next;
            }
        }
...
pub inline fn decodeSymbol(reader: *BitReader, comptime trie: []const TrieNode) !u16 {
    var curr: u16 = 0;
    while ((curr & 0x8000) == 0) {
        const bit = try reader.readBit();
        curr = trie[curr].child[bit];
        if (curr == 0) return error.InvalidHuffmanCode;
    }
    return curr & 0x7FFF;
}

// ISO/IEC 14496-3 Scalefactors Huffman Codebook (121 entries)
pub const SF_CODES = [_]u32{
...
pub const SF_TRIE = buildTrie(&SF_CODES, &SF_BITS);

// Codebook 1: 81 codes, dimension 4, signed alphabet -1..1
pub const CB1_CODES = [_]u32{
...
pub const CB1_TRIE = buildTrie(&CB1_CODES, &CB1_BITS);

// Codebook 2: 81 codes, dimension 4, signed alphabet -1..1
pub const CB2_CODES = [_]u32{
...
pub const CB2_TRIE = buildTrie(&CB2_CODES, &CB2_BITS);

// Codebook 3: 81 codes, dimension 4, unsigned alphabet 0..2
pub const CB3_CODES = [_]u32{
...
pub const CB3_TRIE = buildTrie(&CB3_CODES, &CB3_BITS);

// Codebook 4: 81 codes, dimension 4, unsigned alphabet 0..2
pub const CB4_CODES = [_]u32{
...
pub const CB4_TRIE = buildTrie(&CB4_CODES, &CB4_BITS);

// Codebook 5: 81 codes, dimension 2, signed alphabet -4..4
pub const CB5_CODES = [_]u32{
...
pub const CB5_TRIE = buildTrie(&CB5_CODES, &CB5_BITS);

// Codebook 6: 81 codes, dimension 2, signed alphabet -4..4
pub const CB6_CODES = [_]u32{
...
pub const CB6_TRIE = buildTrie(&CB6_CODES, &CB6_BITS);

// Codebook 7: 64 codes, dimension 2, unsigned alphabet 0..7
pub const CB7_CODES = [_]u32{
...
pub const CB7_TRIE = buildTrie(&CB7_CODES, &CB7_BITS);

// Codebook 8: 64 codes, dimension 2, unsigned alphabet 0..7
pub const CB8_CODES = [_]u32{
...
pub const CB8_TRIE = buildTrie(&CB8_CODES, &CB8_BITS);

// Codebook 9: 169 codes, dimension 2, unsigned alphabet 0..12
pub const CB9_CODES = [_]u32{
...
pub const CB9_TRIE = buildTrie(&CB9_CODES, &CB9_BITS);

// Codebook 10: 169 codes, dimension 2, unsigned alphabet 0..12
pub const CB10_CODES = [_]u32{
...
pub const CB10_TRIE = buildTrie(&CB10_CODES, &CB10_BITS);

// Codebook 11: 289 codes, dimension 2, unsigned alphabet 0..16 + ESCAPE
pub const CB11_CODES = [_]u32{
...
pub const CB11_TRIE = buildTrie(&CB11_CODES, &CB11_BITS);

/// Decodes an escape sequence for values >= 16 in Codebook 11.
pub fn decodeEscape(reader: *BitReader) !i32 {
    var count: usize = 0;
    while ((try reader.readBit()) == 1) {
        count += 1;
    }
    const val = try reader.readBits(u32, count + 4);
    return @intCast((@as(u32, 1) << @as(u5, @intCast(count + 4))) + val);
}

/// Decodes spectral bins for a given codebook into `out_bins` slice.
/// Returns number of bins filled.
pub fn decodeSpectralBins(reader: *BitReader, cb: u4, out_bins: []i32) !usize {
    switch (cb) {
        0 => { // ZERO_HCB
            @memset(out_bins, 0);
            return out_bins.len;
        },
        1 => { // 4-tuples, signed -1..1
            var idx: usize = 0;
            while (idx + 4 <= out_bins.len) : (idx += 4) {
                const sym = try decodeSymbol(reader, &CB1_TRIE);
                var s = sym;
                const z: i32 = @as(i32, @intCast(s % 3)) - 1; s /= 3;
                const y: i32 = @as(i32, @intCast(s % 3)) - 1; s /= 3;
                const x: i32 = @as(i32, @intCast(s % 3)) - 1; s /= 3;
                const w: i32 = @as(i32, @intCast(s % 3)) - 1;
                out_bins[idx + 0] = w;
                out_bins[idx + 1] = x;
                out_bins[idx + 2] = y;
                out_bins[idx + 3] = z;
        ...
        2 => { // 4-tuples, signed -1..1
            var idx: usize = 0;
            while (idx + 4 <= out_bins.len) : (idx += 4) {
                const sym = try decodeSymbol(reader, &CB2_TRIE);
                var s = sym;
                const z: i32 = @as(i32, @intCast(s % 3)) - 1; s /= 3;
                const y: i32 = @as(i32, @intCast(s % 3)) - 1; s /= 3;
                const x: i32 = @as(i32, @intCast(s % 3)) - 1; s /= 3;
        ...
        3 => { // 4-tuples, unsigned 0..2 + signs
            var idx: usize = 0;
            while (idx + 4 <= out_bins.len) : (idx += 4) {
                const sym = try decodeSymbol(reader, &CB3_TRIE);
                var s = sym;
                var z: i32 = @intCast(s % 3); s /= 3;
                var y: i32 = @intCast(s % 3); s /= 3;
                var x: i32 = @intCast(s % 3); s /= 3;
                var w: i32 = @intCast(s % 3);
                if (w > 0 and (try reader.readBit()) == 1) w = -w;
                if (x > 0 and (try reader.readBit()) == 1) x = -x;
                if (y > 0 and (try reader.readBit()) == 1) y = -y;
                if (z > 0 and (try reader.readBit()) == 1) z = -z;
                out_bins[idx + 0] = w;
                out_bins[idx + 1] = x;
                out_bins[idx + 2] = y;
                out_bins[idx + 3] = z;
        ...
        4 => { // 4-tuples, unsigned 0..2 + signs
            var idx: usize = 0;
            while (idx + 4 <= out_bins.len) : (idx += 4) {
                const sym = try decodeSymbol(reader, &CB4_TRIE);
                var s = sym;
                var z: i32 = @intCast(s % 3); s /= 3;
                var y: i32 = @intCast(s % 3); s /= 3;
                var x: i32 = @intCast(s % 3); s /= 3;
        ...
        5 => { // 2-tuples, signed -4..4
            var idx: usize = 0;
            while (idx + 2 <= out_bins.len) : (idx += 2) {
                const sym = try decodeSymbol(reader, &CB5_TRIE);
                const y: i32 = @as(i32, @intCast(sym % 9)) - 4;
                const x: i32 = @as(i32, @intCast(sym / 9)) - 4;
                out_bins[idx + 0] = x;
                out_bins[idx + 1] = y;
            }
            return idx;
        },
        6 => { // 2-tuples, signed -4..4
            var idx: usize = 0;
            while (idx + 2 <= out_bins.len) : (idx += 2) {
                const sym = try decodeSymbol(reader, &CB6_TRIE);
                const y: i32 = @as(i32, @intCast(sym % 9)) - 4;
                const x: i32 = @as(i32, @intCast(sym / 9)) - 4;
                out_bins[idx + 0] = x;
                out_bins[idx + 1] = y;
        ...
        7 => { // 2-tuples, unsigned 0..7 + signs
            var idx: usize = 0;
            while (idx + 2 <= out_bins.len) : (idx += 2) {
                const sym = try decodeSymbol(reader, &CB7_TRIE);
                var y: i32 = @intCast(sym % 8);
                var x: i32 = @intCast(sym / 8);
                if (x > 0 and (try reader.readBit()) == 1) x = -x;
                if (y > 0 and (try reader.readBit()) == 1) y = -y;
                out_bins[idx + 0] = x;
                out_bins[idx + 1] = y;
        ...
        8 => { // 2-tuples, unsigned 0..7 + signs
            var idx: usize = 0;
            while (idx + 2 <= out_bins.len) : (idx += 2) {
                const sym = try decodeSymbol(reader, &CB8_TRIE);
                var y: i32 = @intCast(sym % 8);
                var x: i32 = @intCast(sym / 8);
                if (x > 0 and (try reader.readBit()) == 1) x = -x;
                if (y > 0 and (try reader.readBit()) == 1) y = -y;
        ...
        9 => { // 2-tuples, unsigned 0..12 + signs
            var idx: usize = 0;
            while (idx + 2 <= out_bins.len) : (idx += 2) {
                const sym = try decodeSymbol(reader, &CB9_TRIE);
                var y: i32 = @intCast(sym % 13);
                var x: i32 = @intCast(sym / 13);
                if (x > 0 and (try reader.readBit()) == 1) x = -x;
                if (y > 0 and (try reader.readBit()) == 1) y = -y;
                out_bins[idx + 0] = x;
                out_bins[idx + 1] = y;
        ...
        10 => { // 2-tuples, unsigned 0..12 + signs
            var idx: usize = 0;
            while (idx + 2 <= out_bins.len) : (idx += 2) {
                const sym = try decodeSymbol(reader, &CB10_TRIE);
                var y: i32 = @intCast(sym % 13);
                var x: i32 = @intCast(sym / 13);
                if (x > 0 and (try reader.readBit()) == 1) x = -x;
                if (y > 0 and (try reader.readBit()) == 1) y = -y;
        ...
        11 => { // 2-tuples, unsigned 0..16 + signs + ESCAPE
            var idx: usize = 0;
            while (idx + 2 <= out_bins.len) : (idx += 2) {
                const sym = try decodeSymbol(reader, &CB11_TRIE);
                var x: i32 = @intCast(sym / 17);
                var y: i32 = @intCast(sym % 17);
                const x_sign = if (x != 0) ((try reader.readBit()) == 1) else false;
                const y_sign = if (y != 0) ((try reader.readBit()) == 1) else false;
                if (x == 16) x = try decodeEscape(reader);
                if (y == 16) y = try decodeEscape(reader);
                if (x_sign) x = -x;
                if (y_sign) y = -y;
                out_bins[idx + 0] = x;
                out_bins[idx + 1] = y;
        ...
        else => return error.UnsupportedCodebook,
    }
}
