...
pub const AudioSpecificConfig = config.AudioSpecificConfig;
pub const parseAudioSpecificConfig = config.parseAudioSpecificConfig;
...
pub const AacDecoder = struct {
    ...
    fn applyTns(self: *const AacDecoder, spectrum: *[1024]f32, ics: *const IcsInfo, tns_data: *const TnsData) void {
        tns_mod.applyTns(spectrum, ics, tns_data, self.sample_rate_idx);
    }
