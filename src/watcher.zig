    ...
    while (true) {
        ...
        while (walker.next(io) catch null) |entry| {
            if (entry.kind == .file and std.mem.endsWith(u8, entry.basename, ".mkv")) {
                const full_path = std.fs.path.join(allocator, &.{ config.working_folder, entry.path }) catch continue;
                defer allocator.free(full_path);

                const cache_dir = std.fs.path.join(allocator, &.{ config.working_folder, ".sratim", "manifests" }) catch continue;
                defer allocator.free(cache_dir);
                std.Io.Dir.cwd().createDirPath(io, cache_dir) catch {};

                const hash = std.hash.CityHash64.hash(full_path);
                const cache_file = std.fmt.allocPrint(allocator, "{s}/{d}.mpd", .{ cache_dir, hash }) catch continue;
                defer allocator.free(cache_file);

                const splits_file = std.fmt.allocPrint(allocator, "{s}/{d}.splits", .{ cache_dir, hash }) catch continue;
                ...
                if (std.Io.Dir.cwd().openFile(io, cache_file, .{})) |file| {
                    ...
                    defer url_param_list.deinit(allocator);
                    for (entry.path) |c| {
                        if (c == ' ') {
                            url_param_list.appendSlice(allocator, "%20") catch {};
                        } else if (c == '&') {
                            url_param_list.appendSlice(allocator, "%26") catch {};
                        } else {
                            url_param_list.append(allocator, c) catch {};
                        }
                    }
                    const url_file_param = url_param_list.items;

                    const result = manifest.generateMpd(allocator, full_path, url_file_param) catch |err| {
                        std.debug.print("[Watcher] Failed to generate MPD for {s}: {}\n", .{ entry.path, err });
                        continue;
                    };
                    defer allocator.free(result.xml);
